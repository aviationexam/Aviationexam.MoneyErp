...
public partial class FilterFor<T>
{
   public string Equal(
    ...
    public string Equal<TP>(
        ...
        EFilterOperator.Equal, GetPropertyName(property), value.ToString(format, provider)
    );

    public string Equal(
        Expression<Func<T, bool>> property, bool value
    ) => GetFilterClause(
        EFilterOperator.Equal, GetPropertyName(property), value ? "true" : "false"
    );
